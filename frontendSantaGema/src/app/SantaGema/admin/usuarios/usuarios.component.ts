...
export class UsuariosComponent implements OnInit {
  ...
  constructor(private adminService: AdminService, private messageService: MessageService, private fb: FormBuilder, private spinner: NgxSpinnerService) {
    this.getList();
    this.cols = [
      { field: 'id', header: 'ID' },
  ...
  generarId() {
    this.spinner.show();
    this.adminService.generarDNI().subscribe({
      next: rest => {
        this.cedula = rest['message'];
        this.btnConsultar = false;
        this.spinner.hide();
      }, error: e => {
        console.log(e);
